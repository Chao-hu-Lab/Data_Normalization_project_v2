...
def _lookup_sample_type(
    sample, sample_info_df, col_to_info_row=None, default="UNKNOWN"
):
...
def build_sample_info_mapping(sample_columns, sample_info_df):
    ...
    for sample in unmatched_samples:
        ...
        for idx, info_name in enumerate(info_names):
            ...
            if num_overlap > 0:
                score = 0.8 + 0.2 * (
                    token_overlap / max(len(sample_tokens), len(info_tokens), 1)
                )
...
def enhanced_pqn_normalization(
    data_matrix, sample_info_df, sample_columns, reference_values, col_to_info_row=None
):
    """
    ...
    if qc_count > 0:
        ...
        if (
            qc_count >= VALIDATION_THRESHOLDS["min_qc_samples"]
            and qc_cv_median < CV_QUALITY_THRESHOLDS["acceptable"]
        ):
    ...
    if np.sum(valid_ref_mask) < len(real_reference_values) * 0.5:
        print(
            f"    ⚠ 警告：有效肌酐值不足 50% ({np.sum(valid_ref_mask)}/{len(real_reference_values)})"
        )
    ...
    print(
        f"    - 肌酐範圍: {np.nanmin(real_reference_values):.2f} - {np.nanmax(real_reference_values):.2f}"
    )
    ...
    print(
        f"    - 真實樣本標準化因子範圍: {np.nanmin(normalization_factors_real):.4f} - {np.nanmax(normalization_factors_real):.4f}"
    )
    ...
    if qc_count > 0:
        ...
        print(
            f"    - QC 樣本標準化因子範圍: {np.nanmin(normalization_factors_qc):.4f} - {np.nanmax(normalization_factors_qc):.4f}"
        )
...
def evaluate_group_difference_preservation(
    original_data, normalized_data, sample_info_df, sample_columns, col_to_info_row=None
):
    """
    評估標準化對組間差異的影響
    ...
    # 平均保留率
    avg_preservation = (
        np.mean(np.abs(d_after_valid) / (np.abs(d_before_valid) + 1e-10)) * 100
    )
                        ...
                        flagged_features.append(
                            {
                                ...
                                "change_pct": (
                                    (
                                        np.abs(cohens_d_after[idx])
                                        - np.abs(cohens_d_before[idx])
                                    )
                                    / (np.abs(cohens_d_before[idx]) + 1e-10)
                                )
                                * 100,
    ...
    print(
        f"  - 輕度減弱 (-10% ~ -30%): {mild_reduction} ({mild_reduction / total * 100:.1f}%)"
    )
    print(
        f"  - 顯著減弱 (< -30%): {severe_reduction} ({severe_reduction / total * 100:.1f}%)"
    )
    ...
    if not np.isnan(wilcoxon_pvalue):
        ...
        if wilcoxon_pvalue < 0.05:
            ...
            if median_change_pct < 0:
                print(
                    f"  - 結論: ✗ Cohen's d 中位數顯著下降 ({median_change_pct:.1f}%)"
                )
            else:
                print(
                    f"  - 結論: ✓ Cohen's d 中位數顯著上升 ({median_change_pct:.1f}%)"
                )
        else:
        ...
        if len(flagged_features) > 0:
            print(f"\n  【標記特徵】")
            print(
                f"  顯著改變的特徵數: {len(flagged_features)} ({flagged_ratio * 100:.1f}%)"
            )
...
def plot_qc_variability(original_qc, normalized_qc, qc_names, output_path):
    ...
    ax1.hist(
        cv_before, bins=30, alpha=0.6, color="blue", label="Before", edgecolor="black"
    )
    ax1.hist(
        cv_after, bins=30, alpha=0.6, color="red", label="After", edgecolor="black"
    )

    ax1.axvline(
        x=np.median(cv_before),
        color="blue",
        linestyle="--",
        linewidth=2,
...
def plot_qc_reproducibility(original_qc, normalized_qc, qc_names, output_path):
    ...
    ax2.bar(
        x_pos - width / 2, total_before, width, label="Before", alpha=0.7, color="blue"
    )
    ax2.bar(
        x_pos + width / 2, total_after, width, label="After", alpha=0.7, color="red"
    )

    ax2.axhline(
        y=np.median(total_before), color="blue", linestyle="--", linewidth=1, alpha=0.5
    )
    ax2.axhline(
        y=np.median(total_after), color="red", linestyle="--", linewidth=1, alpha=0.5
    )
...
def get_non_qc_columns(df, sample_info_df):
    ...
    sample_types = (
        sample_info_df.get("Sample_Type", pd.Series([""] * len(sample_info_df)))
        .fillna("")
        .astype(str)
        .str.upper()
    )
...
def get_sample_columns_only(df, sample_info_df):
    ...
    sample_names = sample_info_df.iloc[:, 0].astype(str)
    sample_types = (
        sample_info_df.get("Sample_Type", pd.Series(["Unknown"] * len(sample_info_df)))
        .fillna("Unknown")
        .astype(str)
        .str.upper()
    )
    ...
    for col in df.columns:
        ...
        # 檢查是否包含排除關鍵字
        is_stat_column = any(
            keyword.lower() in str(col).lower() for keyword in exclude_keywords
        )
...
def plot_density_comparison(
    original_data, normalized_data, sample_names, output_path, method_name
):
    """繪製標準化前後的密度圖對比"""
    ...
    ax2.set_title(
        f"After Normalization ({method_name})", fontsize=14, fontweight="bold"
    )
...
def plot_boxplot_comparison(
    ...
    median_before = np.median(
        [np.median(original_log[:, i]) for i in range(len(sample_names))]
    )
    median_after = np.median(
        [np.median(normalized_log[:, i]) for i in range(len(sample_names))]
    )

    sample_medians_before = np.array(
        [np.median(original_log[:, i]) for i in range(len(sample_names))]
    )
    sample_medians_after = np.array(
        [np.median(normalized_log[:, i]) for i in range(len(sample_names))]
    )

    rsd_before = (
        np.std(sample_medians_before, ddof=1) / np.mean(sample_medians_before)
    ) * 100
    rsd_after = (
        np.std(sample_medians_after, ddof=1) / np.mean(sample_medians_after)
    ) * 100
    ...
    total_cv_before = (
        (np.nanstd(original_totals) / np.nanmean(original_totals)) * 100
        if np.nanmean(original_totals) > 0
        else np.nan
    )
    total_cv_after = (
        (np.nanstd(normalized_totals) / np.nanmean(normalized_totals)) * 100
        if np.nanmean(normalized_totals) > 0
        else np.nan
    )
    if np.isfinite(total_cv_before) and total_cv_before != 0:
        total_cv_reduction = (
            (total_cv_before - total_cv_after) / total_cv_before
        ) * 100
    else:
    ...
    try:
        levene_stat, levene_p = levene(
            *[original_log[:, i] for i in range(len(sample_names))]
        )
        if levene_p < 0.001:
            sig_mark = "***"
        elif levene_p < 0.01:
    ...
    ax1.axhline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Median: {median_before:.2f}",
        alpha=0.7,
    )
    ...
    bp2 = ax2.boxplot(
        [normalized_log[:, i] for i in range(len(sample_names))],
        positions=positions,
        widths=0.6,
        patch_artist=True,
        showfliers=False,
    ...
    ax2.axhline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Median: {median_after:.2f}",
        alpha=0.7,
    )
    ...
    ax3.bar(
        ...
        color="#ED553B",
        alpha=0.75,
        label=f"After normalization ({method_name})",
    )

    ax3.axhline(
        y=total_median_before,
        color="#4C72B0",
        linestyle="--",
        linewidth=1.5,
        alpha=0.8,
        label=f"Before median: {total_median_before:.2e}",
    )
    ax3.axhline(
        y=total_median_after,
        color="#ED553B",
        linestyle="--",
        linewidth=1.5,
        alpha=0.8,
        label=f"After median: {total_median_after:.2e}",
    )

    ax3.set_ylabel("Total Intensity", fontsize=12, fontweight="bold")
    ax3.set_title(
        "Sample Total Intensity Overview (linear scale)", fontsize=14, fontweight="bold"
    )
...
def plot_rle(original_data, normalized_data, sample_names, output_path, method_name):
    ...
    # ===== 上圖：Original RLE =====
    bp1 = ax1.boxplot(
        [original_rle[:, i] for i in range(original_rle.shape[1])],
        labels=sample_names,
        patch_artist=True,
        widths=0.6,
        showfliers=False,
    ...
    original_mad = np.median(
        [np.median(np.abs(original_rle[:, i])) for i in range(original_rle.shape[1])]
    )
    ax1.text(
        0.02,
        0.98,
        f"Median Absolute Deviation: {original_mad:.4f}",
    ...
    # ===== 下圖：Normalized RLE =====
    bp2 = ax2.boxplot(
        [normalized_rle[:, i] for i in range(normalized_rle.shape[1])],
        labels=sample_names,
        patch_artist=True,
        widths=0.6,
        showfliers=False,
    ...
    normalized_mad = np.median(
        [
            np.median(np.abs(normalized_rle[:, i]))
            for i in range(normalized_rle.shape[1])
        ]
    )
    improvement = (
        ((original_mad - normalized_mad) / original_mad * 100)
        if original_mad > 0
        else 0
    )
    ...
    ax2.text(
        0.02,
        0.98,
        f"Median Absolute Deviation: {normalized_mad:.4f}\n"
        ...
        bbox=dict(
            boxstyle="round",
            facecolor="lightgreen" if improvement > 0 else "wheat",
            alpha=0.8,
        ),
    )
...
def plot_cv_comparison(original_cv, normalized_cv, output_path, method_name):
    ...
    # Cohen's d (效應量)
    pooled_std = np.sqrt(
        (np.var(original_cv_clean, ddof=1) + np.var(normalized_cv_clean, ddof=1)) / 2
    )
    ...
    # === 1. 標準化前CV%分佈 ===
    ax1.hist(
        original_cv_clean, bins=50, color="steelblue", alpha=0.7, edgecolor="black"
    )
    ax1.axvline(
        x=np.median(original_cv_clean),
        color="red",
        linestyle="--",
        linewidth=2,
        label=f"Median: {np.median(original_cv_clean):.2f}%",
    )
    ...
    ax1.axvline(
        ...
        linestyle=":",
        linewidth=2,
        label=f"Acceptable (<{THRESHOLDS['acceptable']}%)",
        alpha=0.7,
    )
    ax1.axvline(
        x=THRESHOLDS["poor"],
    ...
    ax2.axvline(
        x=np.median(normalized_cv_clean),
        color="red",
        linestyle="--",
        linewidth=2,
        label=f"Median: {np.median(normalized_cv_clean):.2f}%",
    )
    ...
    ax3.axvline(
        x=np.median(cv_improvement),
        color="red",
        linestyle="--",
        linewidth=2,
        label=f"Median: {np.median(cv_improvement):.2f}%",
    )
...
def plot_pca_with_confidence_ellipse(
    ...
    sample_names_clean = [
        sample_names[i] for i in range(len(sample_names)) if valid_samples[i]
    ]
    ...
    if exclude_qc:
        ...
        sample_names_clean = [
            sample_names_clean[i]
            for i in range(len(sample_names_clean))
            if non_qc_mask_all[i]
        ]
    ...
    def simple_permanova(X, groups, n_permutations=999):
        ...
        def calculate_f_stat(dist_mat, grps):
            ...
            ss_total = np.sum(
                [np.sum((X[i] - grand_centroid) ** 2) for i in range(len(X))]
            )
    ...
    if exclude_qc:
        ...
        qc_mean_dist_before, qc_mean_dist_after, qc_dist_reduction_pct = (
            np.nan,
            np.nan,
            np.nan,
        )
        ...
        if np.sum(qc_mask) >= 3:
            ...
            qc_t2_before = np.mean(
                [np.sum((pt - qc_centroid_before) ** 2) for pt in pc_original[qc_mask]]
            )
            qc_t2_after = np.mean(
                [np.sum((pt - qc_centroid_after) ** 2) for pt in pc_normalized[qc_mask]]
            )
            qc_t2_reduction_pct = (
                ((qc_t2_before - qc_t2_after) / qc_t2_before) * 100
                if qc_t2_before > 0
                else 0
            )

            # Mean distance to centroid
            qc_mean_dist_before = np.mean(
                [np.linalg.norm(pt - qc_centroid_before) for pt in pc_original[qc_mask]]
            )
            qc_mean_dist_after = np.mean(
                [
                    np.linalg.norm(pt - qc_centroid_after)
                    for pt in pc_normalized[qc_mask]
                ]
            )
            qc_dist_reduction_pct = (
                ((qc_mean_dist_before - qc_mean_dist_after) / qc_mean_dist_before) * 100
                if qc_mean_dist_before > 0
                else 0
            )
        else:
            qc_t2_before, qc_t2_after, qc_t2_reduction_pct = np.nan, np.nan, np.nan
    ...
    plot_pca_comparison_real_sample_style(
        ...
        sample_names_clean,
        sample_types,
        suptitle=build_pca_comparison_suptitle(
            source_label, result_label, grouping="sample_type"
        ),
...
def plot_confidence_ellipse(
    points, ax, color="blue", label=None, n_std=2.447, linestyle="--", linewidth=2.5
):
    """
...
def evaluate_normalization_quality(original_data, normalized_data):
    ...
    results["cv_improvement_pct"] = (
        (results["cv_improvement"] / results["median_cv_before"]) * 100
        if results["median_cv_before"] > 0
        else 0
    )
    ...
    results["total_cv_before"] = (
        np.std(original_totals) / np.mean(original_totals)
    ) * 100
    results["total_cv_after"] = (
        np.std(normalized_totals) / np.mean(normalized_totals)
    ) * 100
    results["total_cv_improvement"] = (
        results["total_cv_before"] - results["total_cv_after"]
    )
    ...
    results["data_range_after"] = np.nanmax(normalized_data) - np.nanmin(
        normalized_data
    )
...
def create_normalization_summary_report(
    ...
    if pqn_info:
        ...
        report.append(
            f"  - 有效樣本數: {pqn_info['creatinine_valid_count']}/{pqn_info['real_count']}"
        )
    ...
    if not np.isnan(quality_metrics["sample_corr_mean_before"]):
        report.append(f"標準化前:")
        report.append(
            f"  - 平均相關性: {quality_metrics['sample_corr_mean_before']:.4f}"
        )
        report.append(
            f"  - 相關性標準差: {quality_metrics['sample_corr_std_before']:.4f}"
        )
        report.append(f"標準化後:")
        report.append(
            f"  - 平均相關性: {quality_metrics['sample_corr_mean_after']:.4f}"
        )
        report.append(
            f"  - 相關性標準差: {quality_metrics['sample_corr_std_after']:.4f}"
        )
    else:
    ...
    if group_diff_results:
        ...
        report.append(
            f"  - 增強: {group_diff_results['enhanced']} ({group_diff_results['enhanced'] / group_diff_results['total'] * 100:.1f}%)"
        )
        report.append(
            f"  - 穩定 (±10%): {group_diff_results['stable']} ({group_diff_results['stable'] / group_diff_results['total'] * 100:.1f}%)"
        )
        report.append(
            f"  - 輕度減弱 (-10% ~ -30%): {group_diff_results['mild_reduction']} ({group_diff_results['mild_reduction'] / group_diff_results['total'] * 100:.1f}%)"
        )
        report.append(
            f"  - 顯著減弱 (< -30%): {group_diff_results['severe_reduction']} ({group_diff_results['severe_reduction'] / group_diff_results['total'] * 100:.1f}%)"
        )
        report.append(
            f"平均 Effect Size 保留率: {group_diff_results['avg_preservation']:.1f}%"
        )
    ...
    if not np.isnan(quality_metrics["sample_corr_std_before"]):
        if (
            quality_metrics["sample_corr_std_after"]
            < quality_metrics["sample_corr_std_before"]
        ):
    ...
    if quality_metrics["cv_improved_ratio"] > 70:
        report.append(
            f"✓✓ 大多數特徵({quality_metrics['cv_improved_ratio']:.1f}%)的CV%得到改善"
        )
    elif quality_metrics["cv_improved_ratio"] > 50:
        report.append(
            f"✓ 超過半數特徵({quality_metrics['cv_improved_ratio']:.1f}%)的CV%得到改善"
        )
    else:
        report.append(
            f"⚠ 僅{quality_metrics['cv_improved_ratio']:.1f}%的特徵CV%得到改善"
        )
    ...
    # 樣本間相關性改善 (15 分)
    if (
        not np.isnan(quality_metrics["sample_corr_std_before"])
        and quality_metrics["sample_corr_std_after"]
        < quality_metrics["sample_corr_std_before"]
    ):
        score += 15
    ...
    if group_diff_results:
        ...
        # 統計檢驗額外分 (10 分)
        # 1. Wilcoxon 檢驗結果 (5 分)
        if "wilcoxon_pvalue" in group_diff_results and not np.isnan(
            group_diff_results["wilcoxon_pvalue"]
        ):
    ...
    if pqn_info:
        if pqn_info["qc_count"] < 3:
            report.append(
                "⚠ 建議：QC 樣本數量較少，建議至少使用 3 個 QC 樣本以提高標準化穩定性"
            )
        ...
        if pqn_info["creatinine_valid_count"] < pqn_info["real_count"] * 0.9:
            report.append(
                f"⚠ 注意：有 {pqn_info['real_count'] - pqn_info['creatinine_valid_count']} 個樣本缺少有效肌酐值"
            )
    ...
    if group_diff_results:
        if group_diff_results["severe_reduction"] > 0:
            report.append(
                f"⚠ 注意：有 {group_diff_results['severe_reduction']} 個特徵的組間差異顯著減弱"
            )
            report.append("  建議：檢查這些特徵是否與肌酐代謝相關，可能需要特別處理")

        if group_diff_results["avg_preservation"] < 80:
            report.append(
                "⚠ 建議：組間差異保留率較低，建議檢查標準化方法是否適合您的數據"
            )

    # CV% 改善相關建議
    if quality_metrics["cv_improved_ratio"] < 50:
        report.append(
            "⚠ 建議：僅不到一半的特徵 CV% 得到改善，可能需要考慮其他標準化方法"
        )

    # 如果所有指標都良好
    if (
        quality_metrics["cv_improvement_pct"] > 20
        and quality_metrics["total_cv_improvement"] > 10
        and (group_diff_results is None or group_diff_results["avg_preservation"] > 90)
    ):
...
def find_sample_info_sheet(sheets):
    ...
    for sheet_name, df in sheets.items():
        if any(
            col
            for col in df.columns
            if "normalization" in str(col).lower() or "creatinine" in str(col).lower()
        ):
...
def find_correction_column(df):
    ...
    if df.shape[1] < 6:
        ...
        print(f"❌ SampleInfo 欄位不足（需要至少 6 欄，目前只有 {df.shape[1]} 欄）")
        print(f"   現有欄位: {available_cols}")
        print(f"   預期格式: 前 5 欄為樣本資訊，第 F 欄（index 5）起為校正數值")
        print(f"   例如: Creatinine 濃度、Normalization adduct 等數值資料")
        return None, None
    ...
    checked_cols = list(df.columns[5:])
    print(f"❌ 找不到可用於校正的數值欄位")
    print(f"   已檢查的欄位: {checked_cols}")
    print(f"   這些欄位需要包含 >50% 的數值資料（如 Creatinine 濃度）")
    return None, None
...
def perform_normalization(
    ...
    data_matrix = (
        data_df[valid_sample_cols].apply(pd.to_numeric, errors="coerce").values
    )
    ...
    non_qc_mask = np.array(
        [
            ...
            if s in col_to_info_row
            else not any(kw in str(s).upper() for kw in ["QC", "POOLED"])
            for s in sample_columns
        ]
    )
    ...
    print(
        f"✓ 有效參考值數量: {valid_count}/{non_qc_count} (排除 {len(sample_columns) - non_qc_count} 個 QC 樣本)"
    )
    ...
    source_label = get_normalization_source_label(
        source_sheet_name or SHEET_NAMES["qc_batch_scaling"]
    )
    ...
    sample_columns_valid = [
        sample_columns[i] for i in range(len(sample_columns)) if valid_sample_mask[i]
    ]
    ...
    plot_cv_comparison(original_cv, normalized_cv, figure_paths["cv"], method_name)
    ...
    qc_indices_for_cv = [
        i
        for i, s in enumerate(sample_columns)
        if _lookup_sample_type(s, sample_info_df, col_to_info_row) == "QC"
    ]
    if len(qc_indices_for_cv) > 0:
        normalized_df["QC_CV%"] = calculate_cv_per_feature(
            normalized_data[:, qc_indices_for_cv]
        )
    else:
...
def save_normalization_results(
    ...
    try:
        ...
        for r_idx, row in enumerate(
            dataframe_to_rows(cleaned_normalized_df, index=False, header=True), 1
        ):
        ...
        for sheet_name in [preserved_data_sheet_name, sample_info_sheet_name]:
            ...
            for col in ws_original.column_dimensions:
                ws_new.column_dimensions[col].width = ws_original.column_dimensions[
                    col
                ].width
...
def main(input_file=None, session_dir=None):
    ...
    if sample_type_row is not None:
        ...
        normalized_df = insert_sample_type_row(
            normalized_df, sample_type_row, feature_col
        )

    # Build session-aware output path
    if session_dir is not None:
        _save_path = session_output_path(
            session_dir, step=4, prefix=f"Normalized_{method_name}"
        )
    else:
        _save_path = None
    ...
    print(
        "  - 圖檔: Step4_Boxplot_*.png, Step4_CV_*.png, Step4_RLE_*.png, Step4_PCA_*_vs_*_grouped_by_sample_type_*.png"
    )
    ...
    print("=" * 80)
    print(
        f"  中位數CV%改善: {quality_metrics['median_cv_before']:.2f}% → {quality_metrics['median_cv_after']:.2f}%"
    )
    print(
        f"  CV%降低: {quality_metrics['cv_improvement']:.2f}% ({quality_metrics['cv_improvement_pct']:.1f}%)"
    )
    print(f"  改善特徵比例: {quality_metrics['cv_improved_ratio']:.1f}%")
    print(
        f"  樣本總強度CV%: {quality_metrics['total_cv_before']:.2f}% → {quality_metrics['total_cv_after']:.2f}%"
    )

    if not np.isnan(quality_metrics["sample_corr_mean_before"]):
        print(
            f"  樣本間平均相關性: {quality_metrics['sample_corr_mean_before']:.4f} → {quality_metrics['sample_corr_mean_after']:.4f}"
        )
