...
def load_and_process_data(file_path):
    try:
        ...
        if not (file_path.endswith(".xlsx") or file_path.endswith(".xls")):
            raise ValueError(
                f"錯誤：輸入檔案必須是Excel格式 (.xlsx 或 .xls)，但提供了 {file_path}"
            )
        ...
        try:
            ...
            raise ValueError(
                f"錯誤：無法讀取 Excel 檔案，可能已損壞或格式不正確。詳細錯誤: {e}"
            ) from e
        ...
        if missing_sheets:
            raise ValueError(
                f"錯誤：輸入檔案缺少必要的工作表: {', '.join(missing_sheets)}。找到的工作表: {', '.join(all_sheets.keys())}"
            )
        ...
        if missing_cols:
            raise ValueError(
                f"錯誤：'{SHEET_NAMES['sample_info']}' 缺少必要欄位: {', '.join(missing_cols)}。找到的欄位: {', '.join(sample_info_df.columns.tolist())}"
            )
        ...
        if FEATURE_ID_COLUMN in raw_df.columns and FEATURE_ID_COLUMN != "FeatureID":
            ...
            print(
                f"⚠️ 未找到 '{FEATURE_ID_COLUMN}' 或 'FeatureID' 欄位，使用第一欄 '{first_col}' 作為特徵ID"
            )
        ...
        sample_columns = [
            ...
            and col not in {"is_ISTD", "Sample_Type", "sample_type"}
        ]
        ...
        if (
            not raw_df.empty
            and str(raw_df.iloc[0]["FeatureID"]).strip().lower() == "sample_type"
        ):
        ...
        sample_names_in_info = set(
            sample_info_df["Sample_Name"].str.strip().str.lower()
        )
        ...
        if missing_in_raw and missing_in_info:
            ...
            if fuzzy_matched > 0:
                print(
                    f"✓ 樣本名稱模糊匹配成功：{fuzzy_matched} 個樣本（SampleInfo 與 RawIntensity 名稱格式不同但關鍵字匹配）"
                )

            if unmatched_info:
                print(
                    f"⚠️ 警告：以下 {len(unmatched_info)} 個樣本在 SampleInfo 中有記錄，但無法匹配到 RawIntensity:"
                )
                for name in unmatched_info[:5]:
                    print(f"  - {name}")
            if raw_tokens_map:
                print(
                    f"⚠️ 警告：以下 {len(raw_tokens_map)} 個樣本在 RawIntensity 中有數據，但無法匹配到 SampleInfo:"
                )
            ...
            if missing_in_raw:
                print(
                    f"⚠️ 警告：以下樣本在 SampleInfo 中有記錄，但在 RawIntensity 中找不到:"
                )
            ...
            if missing_in_info:
                print(
                    f"⚠️ 警告：以下樣本在 RawIntensity 中有數據，但在 SampleInfo 中找不到:"
                )
        ...
        raw_df[sample_columns] = raw_df[sample_columns].apply(
        ...
        for col in negative_counts_per_col[negative_counts_per_col > 0].index:
            print(
                f"⚠️ 警告：樣本 '{col}' 有 {negative_counts_per_col[col]} 個負值，已設為 0"
            )
        ...
        for col, max_val in extreme_cols.items():
            print(
                f"⚠️ 警告：樣本 '{col}' 有極端高值 ({max_val:.2e})，請檢查數據是否正確"
            )
        ...
        if "FeatureID" in raw_df.columns:
            ...
            if invalid_count > 0:
                print(
                    f"警告：{invalid_count} 筆 'FeatureID' 無法解析為 m/z 和 RT（已設為 NaN）。"
                )

        # 基於 FeatureID 值設定 is_ISTD（避免索引偏移）
        raw_df["is_ISTD"] = (
            raw_df["FeatureID"].astype(str).str.strip().isin(istd_feature_ids)
        )
        ...
        if len(istd_feature_ids) == 0:
            raise ValueError(
                "錯誤：未找到任何 ISTD（請在 RawIntensity 工作表的 FeatureID 欄位中，將內標物質的 FeatureID 標記為紅色字體）"
            )
        elif len(istd_feature_ids) < 3:
            print(
                f"⚠️ 警告：ISTD 數量較少（{len(istd_feature_ids)} 個），建議至少使用 3 個以上的 ISTD"
            )
            print(f"   以確保校正效果的穩定性")
        ...
        for idx, row in istd_in_df.iterrows():
            ...
            if len(values) == 0:
                raise ValueError(
                    f"錯誤：ISTD '{fid}' 的所有樣本強度都是 0 或 NaN，無法進行校正，請檢查數據"
                )
            elif len(values) < len(sample_columns) * 0.5:
                print(
                    f"⚠️ 警告：ISTD '{fid}' 有效值比例較低 ({len(values)}/{len(sample_columns)})"
                )
            ...
            if len(values) >= 2:
                ...
                if cv_percent > CV_QUALITY_THRESHOLDS["acceptable"]:
                    print(
                        f"⚠️ 警告：ISTD '{fid}' 的 CV% 較高 ({cv_percent:.1f}%)，可能影響校正品質"
                    )
        ...
        available_infos = [
            ...
            for info in info_rows
            if info["Sample_Name"]
            not in {v["Sample_Name"] for v in col_to_info.values()}
        ]
        ...
        print(
            f"✓ col_to_info 映射建立完成：{matched_count}/{len(sample_columns)} 個欄位成功匹配到 SampleInfo"
        )
...
def get_qc_sample_columns(raw_df, col_to_info, sample_info_df=None):
    ...
    for col in sample_columns:
        sample_type = normalize_sample_type(
            col_to_info.get(col, {}).get("Sample_Type", "")
        )
        if (
            sample_type == "QC"
            or "QC" in str(col).upper()
            or "POOLED" in str(col).upper()
        ):
...
def evaluate_istd_gate(raw_df, col_to_info, sample_info_df=None):
    ...
    sample_columns, qc_columns = get_qc_sample_columns(
        raw_df, col_to_info, sample_info_df=sample_info_df
    )
...
def find_best_istd_for_analyte(
    analyte_row,
    istd_signals,
    istd_cv,
    sample_columns,  # ✅ 新增參數
    ...
    if valid_sample_cols:
        ...
        numeric_data = istd_signals[valid_sample_cols].apply(
            pd.to_numeric, errors="coerce"
        )
        numeric_data = numeric_data.where(
            numeric_data > 0, np.nan
        )  # Replace <=0 with NaN
        ...
        candidates.append(
            {
                ...
                if not np.isnan(median_intensity)
                else 0.0,
...
def calculate_corrected_ratios(df, sample_info_df):
    ...
    if "Sample_Name" not in sample_info_df.columns:
        raise ValueError(
            f"錯誤：'{SHEET_NAMES['sample_info']}' 缺少 'Sample_Name' 欄位"
        )
    ...
    raw_sample_columns = [
        col for col in all_columns if col not in non_sample and col != "FeatureID"
    ]
    ...
    if len(sample_columns) < len(raw_sample_columns) * 0.5:
        print(
            f"⚠️ 精確名稱匹配僅找到 {len(sample_columns)}/{len(raw_sample_columns)} 個樣本"
        )
        ...
        missing_columns = [
            ...
            if name.strip().lower() not in normalized_columns.values()
        ]
        if missing_columns:
            print(
                f"警告：以下樣本名稱在 '{SHEET_NAMES['raw_intensity']}' 中缺少 (即使大小寫不同): {', '.join(missing_columns)}"
            )
    ...
    for _, analyte_row in analyte_signals.iterrows():
        ...
        for col in sample_columns:
            ...
            corrected = (
                ...
                if istd_intensity > 0
                and not np.isnan(istd_median)
                and not np.isnan(analyte_intensity)
                else np.nan
            )
    ...
    if valid_sample_cols:
        results_df[valid_sample_cols] = results_df[valid_sample_cols].apply(
            pd.to_numeric, errors="coerce"
        )
...
def calculate_qc_cv_with_statistical_test(
    results_df, sample_columns, sample_info_df, original_df, col_to_info=None
):
    """
    ...
    if col_to_info:
        qc_columns = [
            col
            for col in sample_columns
            if normalize_sample_type(col_to_info.get(col, {}).get("Sample_Type", ""))
            == "QC"
        ]
    else:
        qc_samples = sample_info_df[
            sample_info_df["Sample_Type"].str.upper().str.contains("QC")
        ]["Sample_Name"].tolist()
    ...
    valid_qc_cols = [
        c for c in qc_columns if c in results_df.columns and c in original_df.columns
    ]
    ...
    qc_original_data = original_indexed[valid_qc_cols].apply(
        pd.to_numeric, errors="coerce"
    )
    ...
    for row_idx, row_tuple in enumerate(results_df.itertuples()):
        ...
        original_cv = (
            safe_divide(np.std(qc_values_original, ddof=1), orig_mean, np.nan) * 100
        )
        corrected_cv = (
            safe_divide(np.std(qc_values_corrected, ddof=1), corr_mean, np.nan) * 100
        )
        ...
        # ✅ 2. Levene's test（檢驗方差齊性）
        try:
            levene_stat, variance_test_pvalue = levene(
                qc_values_original, qc_values_corrected
            )
        except Exception:
            variance_test_pvalue = np.nan
    ...
    print(
        f"  - 邊緣顯著 (Marginal): {sig_marginal} ({sig_marginal / total_count * 100:.1f}%)"
    )
    ...
    wilcoxon_sig = (
        (cv_results_df["Wilcoxon_pvalue"] < 0.05)
        & (cv_results_df["Wilcoxon_pvalue"].notna())
    ).sum()

    print(f"\n🔬 Wilcoxon 配對符號等級檢定（中位數變化）:")
    print(
        f"  - 成功執行: {wilcoxon_valid}/{total_count} ({wilcoxon_valid / total_count * 100:.1f}%)"
    )
    if wilcoxon_valid > 0:
        print(
            f"  - 中位數顯著改變 (p < 0.05): {wilcoxon_sig}/{wilcoxon_valid} ({wilcoxon_sig / wilcoxon_valid * 100:.1f}%)"
        )
        print(
            f"  - 中位數無顯著改變: {wilcoxon_valid - wilcoxon_sig}/{wilcoxon_valid} ({(wilcoxon_valid - wilcoxon_sig) / wilcoxon_valid * 100:.1f}%)"
        )
    ...
    print(f"\n🔬 Levene's Test（方差齊性）:")
    print(
        f"  - 成功執行: {variance_valid}/{total_count} ({variance_valid / total_count * 100:.1f}%)"
    )
    if variance_valid > 0:
        print(
            f"  - 方差顯著改變 (p < 0.05): {variance_sig}/{variance_valid} ({variance_sig / variance_valid * 100:.1f}%)"
        )
        print(
            f"  - 方差無顯著改變: {variance_valid - variance_sig}/{variance_valid} ({(variance_valid - variance_sig) / variance_valid * 100:.1f}%)"
        )
    ...
    if cv_improvement_valid.sum() > 0:
        ...
        if len(improvements_finite) > 0:
            ...
            print(
                f"  - 範圍: {improvements_finite.min():.2f}% - {improvements_finite.max():.2f}%"
            )

            improved_cv = (improvements_finite > 5).sum()
            similar_cv = (
                (improvements_finite >= -5) & (improvements_finite <= 5)
            ).sum()
            worse_cv = (improvements_finite < -5).sum()

            print(f"\n  改善程度分類:")
            print(
                f"  - 顯著改善 (>5%): {improved_cv} ({improved_cv / len(improvements_finite) * 100:.1f}%)"
            )
            print(
                f"  - 無明顯變化 (±5%): {similar_cv} ({similar_cv / len(improvements_finite) * 100:.1f}%)"
            )
            print(
                f"  - 變差 (<-5%): {worse_cv} ({worse_cv / len(improvements_finite) * 100:.1f}%)"
            )
...
def plot_pvalue_distribution(cv_results_df, plots_dir, timestamp):
    ...
    try:
        ...
        # 直方圖
        ax.hist(
            variance_pvalues, bins=20, color="steelblue", edgecolor="black", alpha=0.7
        )
        ax.axhline(
            y=len(variance_pvalues) / 20,
            color="red",
            linestyle="--",
            linewidth=2,
        ...
        ax.set_title(
            "P-value Distribution (Variance Homogeneity Test)",
            fontsize=14,
            fontweight="bold",
        )
        ...
        pvalue_plot_path = os.path.join(
            plots_dir, f"Step1_Pvalue_Distribution_{timestamp}.png"
        )
...
def perform_pca_analysis_2d(
    ...
    if plots_dir is None:
        ...
        plots_dir = os.path.join(
            base_dir, f"ISTD_Correction_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    for col in sample_columns:
        ...
        color_map[col] = SAMPLE_TYPE_COLORS.get(
            s_type, SAMPLE_TYPE_COLORS.get("Unknown", "#808080")
        )
        marker_map[col] = SAMPLE_TYPE_MARKERS.get(
            s_type, SAMPLE_TYPE_MARKERS.get("Unknown", "x")
        )
    ...
    for left_name, right_name in comparisons:
        ...
        qc_outliers_left = {
            qc_columns[i] for i in range(len(qc_columns)) if outliers_left[i]
        }
        qc_outliers_right = {
            qc_columns[i] for i in range(len(qc_columns)) if outliers_right[i]
        }
        ...
        plot_pca_comparison_qc_style(
            ...
            suptitle=build_pca_comparison_suptitle(
                left_name, right_name, grouping="sample_type"
            ),
        ...
        if np.sum(outliers_left) > 0:
            outlier_samples = [
                qc_columns[i] for i in range(len(outliers_left)) if outliers_left[i]
            ]
            outlier_t2_values = [
                t2_left[i] for i in range(len(outliers_left)) if outliers_left[i]
            ]
            print(f"異常樣本:")
            for sample, t2_val in zip(outlier_samples, outlier_t2_values):
                print(
                    f"  - {sample}: T² = {t2_val:.2f} (閾值 = {t2_threshold_left:.2f})"
                )
        else:
            print("  ✓ 無異常樣本")
        ...
        if np.sum(outliers_right) > 0:
            outlier_samples = [
                qc_columns[i] for i in range(len(outliers_right)) if outliers_right[i]
            ]
            outlier_t2_values = [
                t2_right[i] for i in range(len(outliers_right)) if outliers_right[i]
            ]
            print(f"異常樣本:")
            for sample, t2_val in zip(outlier_samples, outlier_t2_values):
                print(
                    f"  - {sample}: T² = {t2_val:.2f} (閾值 = {t2_threshold_right:.2f})"
                )
        else:
            print("  ✓ 無異常樣本")
        print(f"{'=' * 70}\n")
...
def save_results_to_excel(
    ...
    cv_results_df = calculate_qc_cv_with_statistical_test(
        results_df, sample_columns, sample_info_df, original_df, col_to_info=col_to_info
    )
    ...
    if "Wilcoxon_pvalue" in results_with_cv.columns:
        results_with_cv["Wilcoxon_qvalue"] = apply_fdr_correction(
            results_with_cv["Wilcoxon_pvalue"]
        )
        wilcoxon_valid = results_with_cv["Wilcoxon_pvalue"].notna().sum()
        wilcoxon_sig_p = (
            (results_with_cv["Wilcoxon_pvalue"] < 0.05)
            & (results_with_cv["Wilcoxon_pvalue"].notna())
        ).sum()
        wilcoxon_sig_q = (
            (results_with_cv["Wilcoxon_qvalue"] < 0.05)
            & (results_with_cv["Wilcoxon_qvalue"].notna())
        ).sum()
        print(f"  Wilcoxon test:")
        print(f"    - 有效檢定數: {wilcoxon_valid}")
        print(
            f"    - p < 0.05: {wilcoxon_sig_p} ({wilcoxon_sig_p / wilcoxon_valid * 100:.1f}%)"
        )
        print(
            f"    - q < 0.05 (FDR 校正後): {wilcoxon_sig_q} ({wilcoxon_sig_q / wilcoxon_valid * 100:.1f}%)"
        )

    # 對 Variance Test p-value 進行 FDR 校正
    if "Variance_Test_pvalue" in results_with_cv.columns:
        results_with_cv["Variance_Test_qvalue"] = apply_fdr_correction(
            results_with_cv["Variance_Test_pvalue"]
        )
        variance_valid = results_with_cv["Variance_Test_pvalue"].notna().sum()
        variance_sig_p = (
            (results_with_cv["Variance_Test_pvalue"] < 0.05)
            & (results_with_cv["Variance_Test_pvalue"].notna())
        ).sum()
        variance_sig_q = (
            (results_with_cv["Variance_Test_qvalue"] < 0.05)
            & (results_with_cv["Variance_Test_qvalue"].notna())
        ).sum()
        print(f"  Levene's test:")
        print(f"    - 有效檢定數: {variance_valid}")
        print(
            f"    - p < 0.05: {variance_sig_p} ({variance_sig_p / variance_valid * 100:.1f}%)"
        )
        print(
            f"    - q < 0.05 (FDR 校正後): {variance_sig_q} ({variance_sig_q / variance_valid * 100:.1f}%)"
        )
    ...
    if rename_map:
        ...
        for sheet_name in retained_sheets:
            retained_sheets[sheet_name] = retained_sheets[sheet_name].rename(
                columns=rename_map
            )
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        ...
        _rename_feature_col(results_with_cv).to_excel(
            writer, sheet_name=SHEET_NAMES["istd_correction"], index=False
        )
...
def main(input_file=None, session_dir=None):
    ...
    if input_file is None:
        ...
        input_file = filedialog.askopenfilename(
            title="選擇原始 Excel 檔案", filetypes=[("Excel files", "*.xlsx *.xls")]
        )
    ...
    original_df, sample_info_df, all_sheets, col_to_info = load_and_process_data(
        input_file
    )
    ...
    def _plot_path(filename_without_ext):
        ...
        return os.path.join(
            str(_plots_dir), f"{filename_without_ext}_{run_timestamp}.png"
        )
    ...
    gate_eval = evaluate_istd_gate(
        original_df, col_to_info, sample_info_df=sample_info_df
    )
    print("\n" + "=" * 70)
    print("🔎 ISTD 前置品質檢查")
    print("=" * 70)
    ...
    print(
        f"  - QC_CV% < {CV_QUALITY_THRESHOLDS['excellent']:.0f}% 的 ISTD: {gate_eval['good_istd_count']}"
    )
    ...
    if gate_eval["should_skip"]:
        # Skip: no output file produced; pass original input to downstream steps
