...
# Sheet name constant
QC_LOWESS_ADVANCED_SHEET = SHEET_NAMES.get(
...
def exclude_fallback_istd_rows(data_df, file_path, source_sheet_name):
    ...
    red_marked_feature_ids = collect_red_marked_feature_ids(
        file_path, source_sheet_name
    )
    ...
    istd_mask = (
        data_df["FeatureID"].astype(str).str.strip().isin(red_marked_feature_ids)
    )
    excluded_count = int(istd_mask.sum())
    if excluded_count == 0:
        print(
            "   - RawIntensity 中沒有對應到紅字 ISTD 的資料列，QC-LOWESS 將把所有列視為一般 feature。"
        )
        data_df.attrs["excluded_fallback_istd_count"] = 0
        return data_df
    ...
    print(
        f"   - 偵測到 {excluded_count} 個紅字 ISTD；它們會保留在 'RawIntensity'，但不會進入 'QC LOWESS result' 與 PCA。"
    )
...
def apply_lowess_correction(
    ...
    lowess_result = sm.nonparametric.lowess(
        valid_y, valid_x, frac=frac, it=2, return_sorted=True
    )
    x_fit, y_fit = lowess_result[:, 0], lowess_result[:, 1]

    # 使用全域 QC 中位數（如果有提供），否則使用批次內中位數
    if (
        global_qc_median is not None
        and np.isfinite(global_qc_median)
        and global_qc_median > 0
    ):
    ...
    cv_improvement = (
        original_cv - corrected_cv
        if np.isfinite(original_cv) and np.isfinite(corrected_cv)
        else np.nan
    )
    ...
    qc_predicted = np.where(
        np.isfinite(qc_predicted), qc_predicted, np.nanmedian(valid_y)
    )
...
def perform_lowess_normalization(istd_df, sample_info_df):
    ...
    try:
        source_sheet_name = istd_df.attrs.get(
            "source_sheet_name", SHEET_NAMES["istd_correction"]
        )
        ...
        if (
            "Sample_Name" not in sample_info_df.columns
            or "Sample_Type" not in sample_info_df.columns
        ):
        ...
        sample_info_norm["_norm_name"] = sample_info_norm["Sample_Name"].map(
            normalize_sample_name
        )
        sample_info_norm = sample_info_norm[sample_info_norm["_norm_name"].astype(bool)]
        sample_meta = sample_info_norm.drop_duplicates("_norm_name").set_index(
            "_norm_name"
        )
        col_to_meta = {
        ...
        if missing_meta and len(missing_meta) == len(sample_columns):
            print(
                "⚠️  SampleInfo 與 ISTD_Correction 的樣本名稱格式不同，改用欄位名稱推斷樣本類型"
            )
        elif missing_meta:
        ...
        for sample in sample_columns:
            ...
            if meta_key in sample_meta.index:
                if (
                    "QC"
                    in str(sample_meta.loc[meta_key].get("Sample_Type", "")).upper()
                ):
        ...
        for sample in sample_columns:
            ...
            if sample_type != "QC" and len(batches) != 1:
                raise ValueError(
                    f"Non-QC sample '{sample}' must belong to a single batch"
                )
        ...
        for batch, info in active_batches.items():
            print(
                f"    • Batch {batch}: {len(info['samples'])} samples (QC: {len(info['qc_samples'])})"
            )

        if missing_order_samples:
            print(
                f"⚠️  提示：{len(missing_order_samples)} 個樣本缺少 Injection_Order，已套用臨時序號"
            )
        ...
        if valid_qc_cols:
            qc_data = (
                istd_df[valid_qc_cols].apply(pd.to_numeric, errors="coerce").values
            )
            ...
            feature_cvs = [
                (fid, cv) for fid, cv in zip(feature_ids, cv_values) if not np.isnan(cv)
            ]
        else:
            feature_cvs = []
        ...
        def normalize_feature_for_batch(
            feature_row, batch_name, batch_info, debug_flag, global_median=None
        ):
            ...
            qc_batch_samples = [
                s for s in batch_info["qc_samples"] if s in all_sample_names
            ]
        ...
        valid_global_medians = sum(
            1 for v in global_qc_medians.values() if v is not None
        )
        ...
        for idx, row in istd_df.iterrows():
            ...
            qc_corrected_dict = {
                sample: row[sample] for sample in qc_samples if sample in row.index
            }
            ...
            for batch_name, batch_info in active_batches.items():
                batch_result = normalize_feature_for_batch(
                    row, batch_name, batch_info, debug_flag, global_median
                )
                ...
                if debug_flag and "plot_data" in batch_result:
                    trend_plot_data[(feature_id, batch_name)] = batch_result[
                        "plot_data"
                    ]
            ...
            for sample in qc_corrected_dict:
                if sample in corrected_candidates:
                    qc_corrected_dict[sample] = safe_nanmedian(
                        corrected_candidates[sample]
                    )
                elif sample in fallback_candidates:
                    qc_corrected_dict[sample] = safe_nanmedian(
                        fallback_candidates[sample]
                    )
            ...
            trend_stats.append(
                {
                    ...
                    "MK_Trend_pvalue": safe_nanmedian(
                        [m.get("trend_pvalue", np.nan) for m in trend_metric_buffer]
                    ),
                    "Kendall_Tau": safe_nanmedian(
                        [m.get("trend_tau", np.nan) for m in trend_metric_buffer]
                    ),
                    "LOWESS_R2": safe_nanmedian(
                        [m.get("r_squared", np.nan) for m in trend_metric_buffer]
                    ),
                    "LOWESS_RMSE": safe_nanmedian(
                        [m.get("rmse", np.nan) for m in trend_metric_buffer]
                    ),
        ...
        print("\n  📊 特徵層級統計：")
        print(
            f"     ✅ 全批次均成功: {feature_all_success} ({feature_all_success / len(istd_df) * 100:.1f}%)"
        )
        print(
            f"     ⚠️ 部分批次成功: {feature_partial_success} ({feature_partial_success / len(istd_df) * 100:.1f}%)"
        )
        print(
            f"     ❌ 無成功批次: {feature_no_success} ({feature_no_success / len(istd_df) * 100:.1f}%)"
        )
        ...
        frac_median = (
            float(np.nanmedian(frac_value_list)) if frac_value_list else np.nan
        )
        ...
        print(
            f"     - 高變異策略 (frac=0.8): {high_count} ({frac_pct(high_count):.1f}%)"
        )
        print(
            f"     - 中等變異策略 (frac=0.7): {medium_count} ({frac_pct(medium_count):.1f}%)"
        )
        print(
            f"     - 低變異動態策略 (frac=0.5~0.75): {low_count} ({frac_pct(low_count):.1f}%)"
        )
        if other_count:
            print(
                f"     - 其他（資料不足）: {other_count} ({frac_pct(other_count):.1f}%)"
            )
...
def load_and_process_data(file_path):
    ...
    try:
        ...
        if not (file_path.endswith(".xlsx") or file_path.endswith(".xls")):
            raise ValueError(
                f"輸入檔案必須是 Excel 格式 (.xlsx 或 .xls)，但提供了 {file_path}"
            )
        ...
        sample_info_df = pd.read_excel(
            excel_file, sheet_name=SHEET_NAMES["sample_info"]
        )
        print(
            f"✓ 成功讀取 '{SHEET_NAMES['sample_info']}' 工作表，包含 {len(sample_info_df)} 筆樣本資訊"
        )
        ...
        missing_cols = [
            col for col in required_columns if col not in sample_info_df.columns
        ]
        ...
        if "Batch" not in sample_info_df.columns:
            ...
            print(
                f"⚠️  警告：'{SHEET_NAMES['sample_info']}' 缺少 'Batch' 欄位，已建立預設 Batch1"
            )
        else:
            ...
            if batch_na_mask.any():
                ...
                print(
                    f"⚠️  警告：發現 {batch_na_mask.sum()} 個樣本缺少 Batch，已填入 '{fill_value}'"
                )
        ...
        qc_count = sample_info_df[
            sample_info_df["Sample_Type"].str.upper().str.contains("QC", na=False)
        ].shape[0]
        if qc_count == 0:
            raise ValueError(
                "未找到 QC 樣本（Sample_Type 中無 'QC' 字樣）。QC-LOWESS 校正需要至少 5 個 QC 樣本"
            )
        elif qc_count < 5:
        ...
        if "Injection_Order" in sample_info_df.columns:
            ...
            if not invalid_orders.empty:
                print(
                    f"⚠️  警告：發現 {len(invalid_orders)} 個樣本缺少 Injection_Order:"
                )
            ...
            try:
                ...
                if invalid_count > 0:
                    print(
                        f"⚠️  警告：{invalid_count} 個樣本的 Injection_Order 無法轉換為數值"
                    )
            except Exception as e:
            ...
            if order_na_mask.any():
                ...
                print(
                    f"⚠️  警告：已為缺少 Injection_Order 的樣本指派遞增序號，請於 SampleInfo 中確認"
                )
        ...
        if not duplicate_features.empty:
            print(
                f"⚠️  警告：'{SHEET_NAMES['istd_correction']}' 中發現重複的 FeatureID:"
            )
        ...
        # ===== 防呆12: 樣本欄位檢查 =====
        sample_columns, dropped_columns = identify_sample_columns(
            istd_df, sample_info_df
        )

        if len(sample_columns) == 0:
            raise ValueError(
                f"'{SHEET_NAMES['istd_correction']}' 中沒有匹配 {SHEET_NAMES['sample_info']} 的樣本欄位"
            )
        ...
        if dropped_columns:
            print(
                f"⚠️  警告：偵測到 {len(dropped_columns)} 個推定統計欄位，已自動排除："
            )
        ...
        # ===== 防呆13: 樣本名稱匹配檢查 =====
        sample_names_in_info = set(
            sample_info_df["Sample_Name"].map(normalize_sample_name)
        )
        ...
        if missing_in_istd:
            print(
                f"⚠️  警告：以下樣本在 SampleInfo 中有記錄，但在 {source_sheet_name} 中找不到:"
            )
        ...
        if missing_in_info:
            print(
                f"⚠️  警告：以下樣本在 {source_sheet_name} 中有數據，但在 SampleInfo 中找不到:"
            )
        ...
        for col in sample_columns:
            ...
            if extreme_values > 0:
                ...
                print(
                    f"⚠️  警告：樣本 '{col}' 包含 {extreme_values} 個極端高值 (> 1e12)"
                )
        ...
        if SHEET_NAMES["raw_intensity"] in excel_file.sheet_names:
            try:
                raw_df = pd.read_excel(
                    excel_file, sheet_name=SHEET_NAMES["raw_intensity"]
                )
        ...
        if batch_summary:
            print(
                f"  - 批次分佈: {', '.join([f'{batch}:{count}' for batch, count in batch_summary.items()])}"
            )
        print(f"{'=' * 70}\n")
...
def calculate_qc_cv_with_statistical_test(
    istd_df, lowess_df, sample_columns, sample_info_df, qc_corrected_values
):
    """計算 QC CV% 並進行正確的統計檢定"""
    ...
    for idx in range(len(istd_df)):
        try:
            ...
            # ========== 計算 CV% ==========
            original_cv = (
                np.std(qc_values_istd, ddof=1) / np.mean(qc_values_istd)
            ) * 100
            corrected_cv = (
                np.std(qc_values_lowess, ddof=1) / np.mean(qc_values_lowess)
            ) * 100
    ...
    if len(all_cv_improvements) >= 10:
        try:
            ...
            print(
                f"     改善特徵比例: {positive_improvements}/{len(all_cv_improvements)} ({positive_improvements / len(all_cv_improvements) * 100:.1f}%)"
            )
...
def plot_pvalue_distribution(cv_results_df, plots_dir, timestamp):
    ...
    try:
        ...
        ax.hist(
            levene_pvalues, bins=20, color="steelblue", edgecolor="black", alpha=0.7
        )
        ax.axhline(
            y=len(levene_pvalues) / 20,
            color="red",
            linestyle="--",
            linewidth=2,
        ...
        pvalue_plot_path = os.path.join(
            plots_dir, f"Step2_Pvalue_Distribution_Levene_{timestamp}.png"
        )
        ...
        for feature_id, batch_items in feature_grouped.items():
            ...
            for row_idx, (batch_name, plot_data) in enumerate(batch_items_sorted):
                try:
                    ...
                    ax1.set_title(
                        f"Batch: {batch_name}  |  Raw + LOWESS Fit",
                        fontsize=11,
                        fontweight="bold",
                    )
                    ...
                    print(
                        f"  ⚠️  警告：繪製 {feature_id} / {batch_name} 時發生錯誤: {e}"
                    )
                    continue

            fig.suptitle(
                f"LOWESS Trend Fitting (Feature = {feature_id})",
                fontsize=14,
                fontweight="bold",
                y=0.99,
            )
            ...
            safe_feature = (
                str(feature_id).replace("/", "_").replace("\\", "_").replace(":", "_")
            )
            plot_path = os.path.join(
                plots_dir, f"Step2_Trend_Fitting_Feature_{safe_feature}_{timestamp}.png"
            )
            ...
            if os.path.exists(plot_path):
                ...
                if plot_size > 0:
                    print(
                        f"  ✓ 已保存: {safe_feature} ({n_batches} batches) - {plot_size / 1024:.1f} KB"
                    )
                else:
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
            for sheet_name, df in sheets_to_write:
                _rename_feature_col(df).to_excel(
        ...
        for sheet_name in [source_sheet_name]:
            if (
                sheet_name in original_wb.sheetnames
                and sheet_name in workbook.sheetnames
            ):
                copy_sheet_formatting_only(
                    original_wb[sheet_name], workbook[sheet_name]
                )
        original_wb.close()
        ...
        for sheet_name in [
            ...
            if sheet_name in workbook.sheetnames:
                ...
                for row in worksheet.iter_rows(
                    ...
                    for cell in row:
                        if isinstance(cell.value, (int, float)) and not pd.isna(
                            cell.value
                        ):
                            if (
                                cell.number_format == "General"
                                or cell.number_format == "0"
                            ):
        ...
        if len(cv_improvements) > 0:
            ...
            print(
                f"     範圍: {cv_improvements.min():.2f}% ~ {cv_improvements.max():.2f}%"
            )
            ...
            print(f"\n  分類:")
            print(
                f"     顯著改善 (>5%): {improved} ({improved / len(cv_improvements) * 100:.1f}%)"
            )
            print(
                f"     持平 (±5%): {similar} ({similar / len(cv_improvements) * 100:.1f}%)"
            )
            print(
                f"     變差 (<-5%): {worse} ({worse / len(cv_improvements) * 100:.1f}%)"
            )

        # Levene's test 統計
        levene_valid = cv_results_df["Variance_Test_pvalue"].notna().sum()
        levene_sig = (
            (cv_results_df["Variance_Test_pvalue"] < 0.05)
            & (cv_results_df["Variance_Test_pvalue"].notna())
        ).sum()

        print(f"\n  🔬 Levene's Test（方差齊性）:")
        print(
            f"     成功執行: {levene_valid}/{total_count} ({levene_valid / total_count * 100:.1f}%)"
        )
        if levene_valid > 0:
            print(
                f"     方差顯著改變 (p < 0.05): {levene_sig}/{levene_valid} ({levene_sig / levene_valid * 100:.1f}%)"
            )
        ...
        print(f"\n📊 校正決策統計:")
        print(
            f"  ✅ 全批次成功: {feature_success} ({pct(feature_success, feature_total):.1f}%)"
        )
        print(
            f"  ⚠️ 部分批次成功: {feature_partial} ({pct(feature_partial, feature_total):.1f}%)"
        )
        print(
            f"  ❌ 無成功批次: {feature_no_success} ({pct(feature_no_success, feature_total):.1f}%)"
        )
        ...
        if event_counts:
            total_events = decision_stats.get(
                "total_feature_batch_tasks", sum(event_counts.values())
            )
        ...
        if per_batch_stats:
            ...
            for batch_name, stats_dict in per_batch_stats.items():
                ...
                print(
                    f"    • Batch {batch_name}: 成功 {batch_success}/{batch_total} ({pct(batch_success, batch_total):.1f}%)"
                )
        ...
        if frac_counter:
            ...
            other_count = max(
                total_features - (high_count + medium_count + low_count), 0
            )
            frac_mean = (
                float(np.nanmean(frac_values_global)) if frac_values_global else np.nan
            )
            frac_median = (
                float(np.nanmedian(frac_values_global))
                if frac_values_global
                else np.nan
            )
        ...
        mk_sig = (
            (trend_stats_df["MK_Trend_pvalue"] < 0.05)
            & (trend_stats_df["MK_Trend_pvalue"].notna())
        ).sum()

        print(f"\n📊 進階統計（副表）:")
        print(f"  Mann-Kendall 趨勢檢驗:")
        print(
            f"    成功執行: {mk_valid}/{total_count} ({mk_valid / total_count * 100:.1f}%)"
        )
        if mk_valid > 0:
            print(
                f"    檢測到顯著趨勢 (p < 0.05): {mk_sig}/{mk_valid} ({mk_sig / mk_valid * 100:.1f}%)"
            )
        ...
        print(
            f"  - 副表 ({QC_LOWESS_ADVANCED_SHEET}): Mann-Kendall + R²/RMSE（進階評估）"
        )
        print(f"  - Frac 參數已依代謝物穩定性與 QC 數量動態調整")
        print(
            f"  - 新增 Frac_Used / QC_CV_for_Frac / Frac_Strategy 可於 {QC_LOWESS_ADVANCED_SHEET} 交叉檢視"
        )
...
# ========== Hotelling T² 橢圓繪製 ==========
def draw_hotelling_t2_ellipse(
    ax, scores, alpha=0.05, label=None, edgecolor="black", linestyle="-", linewidth=2.5
):
...
def perform_pca_analysis(
    istd_df, lowess_df, sample_columns, sample_info_df, plots_dir=None, grouping="batch"
):
    ...
    try:
        ...
        if plots_dir is None:
            ...
            plots_dir = os.path.join(
                base_dir, f"QC_LOWESS_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
        ...
        sample_columns_clean = [
            col
            for col in sample_columns_attr
        ...
        qc_indices = [
            i for i, col in enumerate(sample_columns_clean) if col in qc_columns
        ]
        ...
        t2_lowess, t2_threshold_lowess, outliers_lowess = (
            calculate_hotelling_t2_outliers(qc_scores_lowess, scores_lowess, alpha=0.05)
        )

        qc_outlier_map_istd = {
            qc_columns[i]: bool(outliers_istd[i]) for i in range(len(qc_columns))
        }
        qc_outlier_map_lowess = {
            qc_columns[i]: bool(outliers_lowess[i]) for i in range(len(qc_columns))
        }

        print("\n🔍 Hotelling T² 異常值檢測：")
        print(
            f"   {source_label}: {np.sum(outliers_istd)}/{len(qc_columns)} QC 被標記為異常"
        )
        print(
            f"   QC-LOWESS: {np.sum(outliers_lowess)}/{len(qc_columns)} QC 被標記為異常"
        )
        ...
        sample_types = [
            sample_type_map.get(col, "Unknown") for col in sample_columns_clean
        ]
        batch_memberships = [
            tuple(parse_batch_labels(sample_batches.get(col, "Unknown")) or ["Unknown"])
            for col in sample_columns_clean
        ]
        unique_batch_labels = sorted(
            {
                batch
                for memberships in batch_memberships
        ...
        if grouping == "batch" and len(unique_batch_labels) < 2:
            ...
            """
            print(
                f"只有 {len(unique_batch_labels)} 個 batch，跳過依 batch 分組的 PCA 圖。"
            )
            return None

        qc_outliers_left = {
            name for name, is_out in qc_outlier_map_istd.items() if is_out
        }
        qc_outliers_right = {
            name for name, is_out in qc_outlier_map_lowess.items() if is_out
        }
        ...
        plot_pca_comparison_qc_style(
            ...
            suptitle=build_pca_comparison_suptitle(
                source_label, result_label, grouping=grouping_tag
            ),
        ...
        print(f"  解釋變異量:")
        print(
            f"    {source_label}: PC1={var_istd[0] * 100:.2f}%, PC2={var_istd[1] * 100:.2f}%"
        )
        print(
            f"    QC-LOWESS: PC1={var_lowess[0] * 100:.2f}%, PC2={var_lowess[1] * 100:.2f}%"
        )
        print(
            f"  QC outliers: {source_label}={np.sum(outliers_istd)}, QC-LOWESS={np.sum(outliers_lowess)}"
        )
...
def main(input_file=None, session_dir=None):
    ...
    if session_dir is not None:
        ...
        _plots_dir = build_plots_dir(
            "QC_LOWESS_plots", timestamp=timestamp, session_prefix="QC_LOWESS"
        )
    ...
    batch_pca_path = perform_pca_analysis(
        istd_df, lowess_df, sample_columns, sample_info_df, _plots_dir, grouping="batch"
    )
