...
def get_system_fonts():
    ...
    if sys.platform == "darwin":  # macOS
        return {"sans": "Helvetica", "mono": "Monaco"}
    else:  # Windows/Linux
        return {"sans": "Arial", "mono": "Consolas"}
...
class DataNormalizationApp:
        ...
        return [
            {
                ...
                "module": "metabolomics.processors.istd",
                "enabled": True,
            },
            {
                "name": "Step 2: QC Correction",
                "module": "metabolomics.processors.qc_lowess",
                "enabled": False,
            },
            {
                "name": "Step 3: QC Batch Scaling",
                "module": "metabolomics.processors.qc_batch_scaling",
                "enabled": False,
            },
            {
                "name": "Step 4: Conc. Normalization",
                "module": "metabolomics.processors.normalization",
                "enabled": False,
            },
        ]
    ...
    def _create_status_card(self, parent, icon, title, initial_value, value_color=None):
        ...
        label_text = (
            f"{icon} {title}" if isinstance(icon, str) and icon.isascii() else title
        )
    ...
    def _set_default_split_ratio(self):
        ...
        if total_width <= 0:
            self.master.after(
                workspace_defaults["initial_retry_ms"], self._set_default_split_ratio
            )
            return
    ...
    def _set_step_status(self, index, state):
        if not hasattr(self, "step_status_labels") or index >= len(
        ...
        status_styles = {
            "idle": {
                ...
                "bg": "#eef2f7",
                "fg": self.color_scheme.get(
                    "text_light", self.color_scheme.get("ghost_text", "#6b7280")
                ),
            },
            "running": {
    ...
    def _invalidate_step_and_downstream(self, step_name):
        ...
        for index in range(start_index, len(self.steps)):
            ...
            if hasattr(self, "step_excel_buttons") and index < len(
                self.step_excel_buttons
            ):
                self.step_excel_buttons[index].config(state="disabled")
            if hasattr(self, "step_plot_buttons") and index < len(
    ...
    def configure_styles(self):
        ...
        self.style.configure(
            "Header.TFrame", background=self.color_scheme["background"], relief="flat"
        )

        # Hero Frame (檔案選擇區域 - 主色調)
        self.style.configure(
            "Hero.TFrame", background=self.color_scheme["hero_bg"], relief="flat"
        )
        ...
        self.style.configure("TNotebook.Tab", font=(FONTS["sans"], 11), padding=[12, 6])
    ...
    def create_hero_section(self):
        ...
        hero_inner = tk.Frame(
            hero_container, bg=self.color_scheme["hero_bg"], padx=18, pady=16
        )
    ...
    def create_split_layout(self):
        ...
        self.split_frame.add(
            self.left_frame,
            minsize=workspace_defaults["left_minsize"],
            stretch="always",
        )
        ...
        self.split_frame.add(
            self.right_frame,
            minsize=workspace_defaults["right_minsize"],
            stretch="always",
        )
        self.master.after(
            workspace_defaults["initial_retry_ms"], self._set_default_split_ratio
        ...
        if workspace_defaults.get("keep_ratio_on_resize"):
            self.split_frame.bind(
                "<Configure>",
                lambda _event: self.master.after_idle(self._set_default_split_ratio),
            )
    ...
    def create_steps_area(self):
        ...
        for i, step in enumerate(steps):
            ...
            # === 左側：顏色指示條 + 步驟編號 ===
            left_indicator = tk.Frame(
                card_inner,
                bg=step["accent"],
                width=card_tokens["badge_width"],
                height=104,
            )
            ...
            input_source_frame = tk.Frame(
                info_section, bg=self.color_scheme["panel_bg"]
            )
    ...
    def create_right_panel(self):
        ...
        stats_inner = tk.Frame(
            stats_tab, bg=self.color_scheme["panel_bg"], padx=20, pady=20
        )
    ...
    def create_progress_area(self):
        ...
        self.progress_frame = tk.Frame(
            self.main_frame, bg=self.color_scheme["background"], height=88
        )
        ...
        content = tk.Frame(
            progress_inner, bg=self.color_scheme["panel_bg"], padx=16, pady=10
        )
        ...
        self.progress_bar = ttk.Progressbar(
            content, mode="determinate", maximum=100, length=400
        )
            ...
            if can_execute:
                ...
                if step["name"] not in completed_steps and i == next_step_index:
                    ...
                    if hasattr(self, "step_cards") and i < len(self.step_cards):
                        self.step_cards[i].config(
                            highlightbackground=step["accent"], highlightthickness=2
                        )
                else:
                    # 已完成步驟 (Ghost 樣式)
                    self.step_buttons[i].config(
                ...
                if "output_path" in data_dict:
                    ...
                    if self.current_stats["step_name"]:
                        if isinstance(data, ProcessingResult):
                            ...
                            self.step_outputs[self.current_stats["step_name"]] = (
                                data_dict
                            )
        ...
        for i, label in enumerate(self.step_input_labels):
            if i == 0:
                ...
                output_path = self._get_output_path(
                    self.step_outputs.get(prev_step_name)
                )
                if output_path:
                    output_filename = os.path.basename(output_path)
                    label.config(
                        text=f"← {output_filename}", fg=self.steps[i]["accent"]
                    )
                else:
                    label.config(
                        text=f"Output from Step {i}", fg=self.color_scheme["text_light"]
                    )
    ...
    def open_step_plots(self, step):
        ...
        if self.current_session_dir is not None:
            ...
            if os.path.isdir(plots_dir):
                self._open_path_in_system(
                    plots_dir, "Plot folder does not exist", "Cannot open folder"
                )
            else:
    ...
    def _set_input_file(self, file_path):
        ...
        self.input_file_label.config(text=filename, fg=self.color_scheme["text_dark"])
    ...
    def export_to_metaboanalyst(self):
        ...
        if step4_name not in self.step_outputs:
            messagebox.showwarning(
                "No Output", "Step 4 (Conc. Normalization) has not been completed yet."
            )
            return
        ...
        try:
            ...
            result_path = Path(
                convert_dnp_to_metaboanalyst(step4_output, str(output_path))
            )
    ...
    def run_all_steps(self):
        ...
        if messagebox.askyesno(
            "Confirm", "Are you sure you want to run all steps automatically?"
        ):
    ...
    def execute_step(self, step):
        ...
        self.current_thread = threading.Thread(
            target=self.run_step, args=(step,), daemon=True
        )
    ...
    def run_step(self, step):
        ...
        try:
            try:
                ...
                error_msg = str(exc)
                self.master.after(
                    0, lambda s=step, err=error_msg: self.on_step_cancelled(s, err)
                )
                return

            if step["name"] != "Step 1: ISTD Correction":
                self.logger.info(
                    f"Auto-selected previous output: {os.path.basename(current_input)}"
                )
            ...
            if isinstance(result, ProcessingResult):
                ...
                self.logger.warning(
                    f"{step['name']} returned None, trying to extract info from logs"
                )
            ...
            self.logger.error(traceback.format_exc())
            self.master.after(
                0, lambda s=step, err=error_msg: self.on_step_error(s, err)
            )
    ...
    def on_step_start(self, step):
        ...
        self.set_progress(
            f"{step['name']} Running...", value=start_progress, running=True
        )
    ...
    def _build_skip_guidance(self, step_name, skip_reason):
        ...
        if (
            step_name == "Step 1: ISTD Correction"
            and skip_reason == "insufficient_good_istd"
        ):
            return (
    ...
    def on_step_complete(self, step, result):
        ...
        # Enable buttons only for non-skipped steps
        if not was_skipped:
        ...
        self.logger.info(
            f"Execution Time: {self.current_stats['execution_time']:.2f} s"
        )
        ...
        self.set_progress(
            f"{step['name']} Completed", value=end_progress, running=False
        )
        ...
        if self.auto_run_mode:
            ...
            if next_index < len(self.steps):
                self.logger.info(
                    f"Auto-running next step in 1s: {self.steps[next_index]['name']}"
                )
                self.master.after(
                    1000, lambda: self.execute_step(self.steps[next_index])
                )
            else:
                self.auto_run_mode = False
    ...
    def on_step_error(self, step, error):
        ...
        retry = messagebox.askyesno(
            "Execution Failed", f"{step['name']} Failed:\n{error}\n\nRetry?"
        )
    ...
    def reset_all_steps(self):
        ...
        if self.is_executing:
            messagebox.showwarning(
                "Cannot Reset", "Task is running, please cancel first"
            )
            return

        if messagebox.askyesno(
            "Confirm Reset", "Are you sure you want to reset all steps?"
        ):
            ...
            self.input_file_label.config(
                text="No file selected...", fg=self.color_scheme["text_light"]
            )
            self.selected_file_path = None
