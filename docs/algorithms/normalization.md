...
本工具採用的是**混合標準化策略**，這個策略結合了兩種互補的方法：首先使用樣本特異性校正（通常是肌酐校正，針對尿液樣本）來處理個體間的濃度差異，然後使用 PQN (Probabilistic Quotient Normalization) 來處理整體代謝組學譜的尺度差異。這種雙重策略能夠同時處理「個體特異性變異」和「群體系統性變異」，比單一方法更全面。
...
校正後，樣本 A 的代謝物強度會乘以 2.0（100/50），樣本 B 會乘以 0.5（100/200），使兩者回到相同的「標準濃度」基準。
