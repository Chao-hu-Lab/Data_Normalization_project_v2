import numpy as np
import pandas as pd

# 設定隨機種子以確保可重現性
np.random.seed(51)

# 參數設定
n_features = 50
n_control = 20  # 控制組樣本數
n_exposed = 20  # 暴露組樣本數
n_samples_total = n_control + n_exposed

# 隨機選擇5-8個特徵作為內標
n_internal_standards = np.random.randint(5, 9)
is_indices = np.random.choice(n_features, n_internal_standards, replace=False)
is_indices = sorted(is_indices)

# 隨機選擇10-15個特徵作為差異表現特徵（在控制組和暴露組之間有顯著差異）
n_differential_features = np.random.randint(10, 16)
non_is_features = [j for j in range(n_features) if j not in is_indices]
diff_indices = np.random.choice(non_is_features, n_differential_features, replace=False)
diff_indices = sorted(diff_indices)

print(f"選定的內標特徵索引: {is_indices}")
print(f"內標數量: {n_internal_standards}")
print(f"\n選定的差異表現特徵索引: {diff_indices}")
print(f"差異特徵數量: {n_differential_features}")

# 生成樣本序列：QC + 控制組 + 暴露組，每3個樣本插入一個QC
sample_list = []
group_list = []
sample_type_list = []

qc_counter = 1
control_counter = 1
exposed_counter = 1

# 第一針是QC
sample_list.append(f'QC{qc_counter}')
group_list.append('QC')
sample_type_list.append('QC')
qc_counter += 1

# 交替添加控制組和暴露組樣本
sample_count = 0
while control_counter <= n_control or exposed_counter <= n_exposed:
    # 添加最多3個樣本
    samples_this_block = 0
    for _ in range(4):
        # 交替添加控制組和暴露組
        if control_counter <= n_control and (control_counter <= exposed_counter or exposed_counter > n_exposed):
            sample_list.append(f'Control_{control_counter}')
            group_list.append('Control')
            sample_type_list.append('Sample')
            control_counter += 1
            sample_count += 1
            samples_this_block += 1
        elif exposed_counter <= n_exposed:
            sample_list.append(f'Exposed_{exposed_counter}')
            group_list.append('Exposed')
            sample_type_list.append('Sample')
            exposed_counter += 1
            sample_count += 1
            samples_this_block += 1
        
        if control_counter > n_control and exposed_counter > n_exposed:
            break
    
    # 確保即使所有樣本都已加入，仍然添加QC
    if samples_this_block > 0:
        sample_list.append(f'QC{qc_counter}')
        group_list.append('QC')
        sample_type_list.append('QC')
        qc_counter += 1


n_samples = len(sample_list)

# 生成基礎矩陣 (樣本 × 特徵)
base_intensity = 1000000
data_matrix = np.random.uniform(500000, 1500000, (n_samples, n_features))

# 為QC樣本設定相似的基礎值
qc_base_values = np.random.uniform(800000, 1200000, n_features)
for i, sample_type in enumerate(sample_type_list):
    if sample_type == 'QC':
        data_matrix[i, :] = qc_base_values * np.random.uniform(0.95, 1.05, n_features)

# === 為內標設定特殊的穩定基礎值 ===
is_base_values = np.random.uniform(900000, 1100000, n_internal_standards)
for idx, is_idx in enumerate(is_indices):
    stable_value = is_base_values[idx]
    data_matrix[:, is_idx] = stable_value * np.random.uniform(0.97, 1.03, n_samples)

# === 為差異表現特徵添加組間差異 ===
for diff_idx in diff_indices:
    for i, group in enumerate(group_list):
        if group == 'Control':
            # 控制組：保持原值
            pass
        elif group == 'Exposed':
            # 暴露組：隨機上調或下調 1.5-3倍
            if np.random.random() > 0.5:
                # 上調
                fold_change = np.random.uniform(1.5, 3.0)
            else:
                # 下調
                fold_change = np.random.uniform(0.33, 0.67)
            data_matrix[i, diff_idx] *= fold_change

# 1. 添加時間相關的儀器飄移 (2-3.5%)
for i in range(n_samples):
    drift_rate = np.random.uniform(0.02, 0.035)
    time_factor = 1 + (drift_rate * i / n_samples)
    feature_drift = time_factor * np.random.uniform(0.99, 1.01, n_features)
    
    # 內標對儀器飄移的響應更小 (±0.5%)
    for is_idx in is_indices:
        feature_drift[is_idx] = time_factor * np.random.uniform(0.995, 1.005)
    
    data_matrix[i, :] *= feature_drift

# 2. 添加異常值 (只在非QC、非內標的樣本和特徵中)
sample_indices = [i for i, st in enumerate(sample_type_list) if st == 'Sample']
non_is_features = [j for j in range(n_features) if j not in is_indices]
n_outliers = np.random.randint(8, 15)

for _ in range(n_outliers):
    sample_idx = np.random.choice(sample_indices)
    feature_idx = np.random.choice(non_is_features)
    
    if np.random.random() > 0.5:
        # 高強度異常 (1.5個order = 10^1.5 ≈ 31.6倍)
        data_matrix[sample_idx, feature_idx] *= 31.6
    else:
        # 低強度異常 (但不低於1000)
        current_value = data_matrix[sample_idx, feature_idx]
        data_matrix[sample_idx, feature_idx] = max(1000, current_value * 0.3)

# 3. 添加缺失值
for j in range(n_features):
    if j in is_indices:
        # 內標的缺失率非常低 (0-5%)
        missing_rate = np.random.uniform(0, 0.05)
    else:
        # 一般特徵的缺失率 (0-29%)
        missing_rate = np.random.uniform(0, 0.29)
    
    n_missing = int(n_samples * missing_rate)
    if n_missing > 0:
        # 優先在樣本中添加缺失值
        available_indices = sample_indices.copy()
        if n_missing <= len(available_indices):
            missing_indices = np.random.choice(available_indices, n_missing, replace=False)
        else:
            missing_indices = np.random.choice(n_samples, n_missing, replace=False)
        data_matrix[missing_indices, j] = np.nan

# 確保所有值不低於1000（除了NaN）
data_matrix = np.where((~np.isnan(data_matrix)) & (data_matrix < 1000), 1000, data_matrix)

# 創建 DataFrame (轉置：特徵為行，樣本為列)
feature_names = []
feature_types = []
for j in range(n_features):
    if j in is_indices:
        feature_names.append(f'IS_{j+1}')
        feature_types.append('Internal_Standard')
    elif j in diff_indices:
        feature_names.append(f'Feature_{j+1}')
        feature_types.append('Differential')
    else:
        feature_names.append(f'Feature_{j+1}')
        feature_types.append('Analyte')

df = pd.DataFrame(data_matrix.T, index=feature_names, columns=sample_list)

# 創建特徵資訊 DataFrame
feature_info = pd.DataFrame({
    'Feature': feature_names,
    'Type': feature_types,
    'Index': range(n_features)
})

# 創建樣本資訊 DataFrame
sample_info = pd.DataFrame({
    'Sample': sample_list,
    'Group': group_list,
    'Type': sample_type_list,
    'Injection_Order': range(1, n_samples + 1)
})

# 顯示結果摘要
print("\n" + "=" * 70)
print("data generation complete!")
print("=" * 70)
print(f"\n矩陣維度: {df.shape} (特徵 × 樣本)")
print(f"總樣本數: {n_samples}")
print(f"總特徵數: {n_features}")
print(f"內標數量: {n_internal_standards}")
print(f"差異特徵數量: {n_differential_features}")

print(f"\n特徵類型統計:")
print(feature_info['Type'].value_counts())

print(f"\n內標列表:")
is_features = feature_info[feature_info['Type'] == 'Internal_Standard']
print(is_features)

print(f"\n差異表現特徵列表:")
diff_features = feature_info[feature_info['Type'] == 'Differential']
print(diff_features)

print(f"\n樣本類型統計:")
print(sample_info['Type'].value_counts())

print(f"\n組別分布:")
group_summary = sample_info.groupby(['Group', 'Type']).size().unstack(fill_value=0)
print(group_summary)

# 計算每個特徵的缺失率和CV
missing_rates = pd.Series(
    [df.loc[fname].isna().sum() / n_samples * 100 for fname in feature_names],
    index=feature_names
)
cv_values = pd.Series(
    [df.loc[fname].std() / df.loc[fname].mean() * 100 for fname in feature_names],
    index=feature_names
)

print(f"\n缺失值統計:")
print(f"總缺失值數量: {df.isna().sum().sum()}")

# 使用特徵名稱來篩選
analyte_features = feature_info[feature_info['Type'] == 'Analyte']['Feature'].tolist()
is_feature_names = feature_info[feature_info['Type'] == 'Internal_Standard']['Feature'].tolist()
diff_feature_names = feature_info[feature_info['Type'] == 'Differential']['Feature'].tolist()

print(f"一般特徵平均缺失率: {missing_rates[analyte_features].mean():.2f}%")
print(f"差異特徵平均缺失率: {missing_rates[diff_feature_names].mean():.2f}%")
print(f"內標平均缺失率: {missing_rates[is_feature_names].mean():.2f}%")

print(f"\n變異係數(CV)統計:")
print(f"一般特徵平均CV: {cv_values[analyte_features].mean():.2f}%")
print(f"差異特徵平均CV: {cv_values[diff_feature_names].mean():.2f}%")
print(f"內標平均CV: {cv_values[is_feature_names].mean():.2f}%")

print(f"\n強度範圍:")
print(f"最小值: {np.nanmin(data_matrix):.2f}")
print(f"最大值: {np.nanmax(data_matrix):.2f}")
print(f"中位數: {np.nanmedian(data_matrix):.2f}")

print("\n各組別強度中位數比較 (以第一個內標為例):")
is_feature_name = is_features.iloc[0]['Feature']
for group in ['Control', 'Exposed', 'QC']:
    group_samples = sample_info[sample_info['Group'] == group]['Sample'].tolist()
    if len(group_samples) > 0:
        group_values = df.loc[is_feature_name, group_samples].dropna()
        if len(group_values) > 0:
            print(f"  {group}: {group_values.median():.2f} (CV: {(group_values.std()/group_values.mean()*100):.2f}%)")

print("\n各組別強度中位數比較 (以第一個差異特徵為例):")
if len(diff_features) > 0:
    diff_feature_name = diff_features.iloc[0]['Feature']
    for group in ['Control', 'Exposed']:
        group_samples = sample_info[sample_info['Group'] == group]['Sample'].tolist()
        group_values = df.loc[diff_feature_name, group_samples].dropna()
        if len(group_values) > 0:
            print(f"  {group}: {group_values.median():.2f} (CV: {(group_values.std()/group_values.mean()*100):.2f}%)")
    
    # 計算fold change
    control_samples = sample_info[sample_info['Group'] == 'Control']['Sample'].tolist()
    exposed_samples = sample_info[sample_info['Group'] == 'Exposed']['Sample'].tolist()
    control_median = df.loc[diff_feature_name, control_samples].dropna().median()
    exposed_median = df.loc[diff_feature_name, exposed_samples].dropna().median()
    fold_change = exposed_median / control_median
    print(f"  Fold Change (Exposed/Control): {fold_change:.2f}")

print("\n內標穩定性分析:")
for _, row in is_features.iterrows():
    feature_name = row['Feature']
    values = df.loc[feature_name].dropna()
    cv = values.std() / values.mean() * 100
    missing_rate = df.loc[feature_name].isna().sum() / n_samples * 100
    print(f"  {feature_name}: CV={cv:.2f}%, 缺失率={missing_rate:.2f}%")

print("\n前5個特徵 × 前10個樣本的數據預覽:")
print(df.iloc[:5, :10].round(2))

print("\n樣本資訊 (前20個):")
print(sample_info.head(20))

# 保存到檔案
df.to_csv('feature_matrix_control_exposed.csv')
sample_info.to_csv('sample_info_control_exposed.csv', index=False)
feature_info.to_csv('feature_info_control_exposed.csv', index=False)

# 保存統計摘要
stats_summary = pd.DataFrame({
    'Feature': feature_names,
    'Type': feature_types,
    'Mean': [df.loc[fname].mean() for fname in feature_names],
    'Median': [df.loc[fname].median() for fname in feature_names],
    'SD': [df.loc[fname].std() for fname in feature_names],
    'CV(%)': cv_values.values,
    'Missing_Rate(%)': missing_rates.values,
    'Min': [df.loc[fname].min() for fname in feature_names],
    'Max': [df.loc[fname].max() for fname in feature_names]
})

# 為差異特徵添加fold change資訊
control_samples = sample_info[sample_info['Group'] == 'Control']['Sample'].tolist()
exposed_samples = sample_info[sample_info['Group'] == 'Exposed']['Sample'].tolist()

fold_changes = []
for fname in feature_names:
    control_median = df.loc[fname, control_samples].dropna().median()
    exposed_median = df.loc[fname, exposed_samples].dropna().median()
    if control_median > 0:
        fc = exposed_median / control_median
    else:
        fc = np.nan
    fold_changes.append(fc)

stats_summary['Fold_Change_Exposed_vs_Control'] = fold_changes
stats_summary.to_csv('feature_statistics_control_exposed.csv', index=False)

print("\n" + "=" * 70)
print("已保存檔案:")
print("- feature_matrix_control_exposed.csv (特徵矩陣)")
print("- sample_info_control_exposed.csv (樣本資訊)")
print("- feature_info_control_exposed.csv (特徵資訊，標註內標和差異特徵)")
print("- feature_statistics_control_exposed.csv (特徵統計摘要，含Fold Change)")
print("=" * 70)
